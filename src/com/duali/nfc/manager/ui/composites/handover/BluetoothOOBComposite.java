package com.duali.nfc.manager.ui.composites.handover;
...
import com.duali.nfc.ndef.records.BluetoothOOBDataRecord.ServiceClassUUIDList;
import com.duali.nfc.ndef.records.BluetoothOOBDataRecord.ServiceOfDevice;
import com.duali.nfc.ndef.records.handover.OOBData;
import com.duali.utils.Hex;

public class BluetoothOOBComposite extends AbstractRecordComposite {
	 ...
	 * @param style
	 */
	public BluetoothOOBComposite(Composite parent, int style) {
		super(parent, style);
		setLayout(null);
	...
	private void createName() {
		...
		lblName.setBounds(10, 47, 103, 15);
		lblName.setText("Device Name");
		lblName.setFont(FontUtils.FONT_HELVETICAS_BOLD_SIZE8);
		lblName.setForeground(DisplayUtils.COLOR_TAGMAN_HEAD_DARK_GRAY);		
		
		txtName = new Text(this, SWT.BORDER);
		txtName.setBounds(125, 41, 210, 21);
		txtName.setTextLimit(20);
		txtName.setFont(FontUtils.FONT_HELVETICAS_BOLD_SIZE8);
		txtName.setForeground(DisplayUtils.COLOR_TAGMAN_HEAD_DARK_GRAY);
	}
	
	private void createMacAddress() {
...
//					System.out.println("");					
	...
	public void addRecord(TableViewer tbvRecord) {
		...
		OOBData oobData1 = null;
		if (deviceName != null)
			oobData1 = new OOBData(EIR.COMPLETE_LOCAL_NAME, deviceName.getBytes());
		
		OOBData oobData2 = new OOBData(EIR.COMPLETE_CLASS_OF_DEVICE, new byte[]{ServiceOfDevice.WEARABLE_HEADSET_DEVICE,
				ServiceOfDevice.AUDIO_VIDEO, ServiceOfDevice.AUDIO});

		byte[] data = new byte[ServiceClassUUIDList.HSP.length + ServiceClassUUIDList.A2DP.length];
		System.arraycopy(ServiceClassUUIDList.HSP, 0, data, 0, ServiceClassUUIDList.HSP.length);
		System.arraycopy(ServiceClassUUIDList.A2DP, 0, data, ServiceClassUUIDList.HSP.length, ServiceClassUUIDList.A2DP.length);
		OOBData oobData3 = new OOBData(EIR.COMPLETE_16BIT_SERVICE_CLASS_UUID_LIST, data);

		OOBData[] options = null;
		if (oobData1 != null)
			options = new OOBData[]{oobData1, oobData2, oobData3};
		else 
			options = new OOBData[]{oobData2, oobData3};
		
		BluetoothOOBDataRecord btOOBDataRecord = new BluetoothOOBDataRecord(macAddress, options);
		
		addRecord(tbvRecord, btOOBDataRecord);
	}
...
//		sb.append("-");
		...
		return Hex.hexStringToBytes(sb.toString());
	}
}
